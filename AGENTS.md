...
- `write.jl` writes the lines digit by digit (no Printf): every TLE field is an integer or a fixed-point number scaled to an integer. `convert(String, tle)` wraps `write_tle`.
