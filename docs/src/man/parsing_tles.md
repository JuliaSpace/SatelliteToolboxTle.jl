...
If a single TLE cannot be parsed, the functions [`read_tle`](@ref), [`@tle_str`](@ref), and
[`@tle_nc_str`](@ref) throw a [`TleParseError`](@ref) describing the problem and, when
known, the line of the input where it was found:
...
When parsing multiple TLEs, the invalid ones are skipped with a warning, and the valid
ones are returned:
